(() => {
  ...
  const workflowProxyConfigForSpawn = (command, args, rest) => {
    ...
    const wrapper = environment.CODEX_CLI_PATH;
    const target = environment.CODEY_CODEX_CLI_WRAPPER_TARGET;
    const managedWrapper = typeof command === "string" && path.isAbsolute(command)
      && typeof wrapper === "string" && path.isAbsolute(wrapper)
      && normalizeExecutable(command) === normalizeExecutable(wrapper)
      && typeof target === "string" && path.isAbsolute(target)
      && isExactDirectCodexAppServerSpawn(target, args);
    if (!isExactDirectCodexAppServerSpawn(command, args) && !managedWrapper) return null;
    ...
    if (!managedWrapper && normalizeExecutable(executable) === normalizeExecutable(String(command))) {
        return null;
    }
