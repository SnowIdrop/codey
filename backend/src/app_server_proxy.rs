...
impl ControlConfig {
    fn from_environment() -> Result<Option<Self>> {
        ...
        let address = env::var(ENV_CONTROL_ADDRESS)
            .context("工作流代理缺少控制地址")?
            .parse()
            .context("工作流代理控制地址无效")?;
        if !is_private_control_address(address) {
            bail!("工作流代理控制地址必须是本机回环地址");
        }
        let capability_token = env::var(ENV_CAPABILITY_TOKEN)
            .context("工作流代理缺少能力令牌")?
            .into_bytes();
...
async fn run_proxy(config: HelperConfig) -> Result<i32> {
    let control = if let Some(control) = config.control {
        Some((
            TcpListener::bind(control.address)
                .await
                .context("无法绑定工作流代理控制端口")?,
    ...
    let control_task = control.map(|(listener, capability_token)| {
...
fn validate_real_codex_invocation(executable: &OsStr, arguments: &[OsString]) -> Result<()> {
    ...
    if !matches_codex_executable_name(executable_name) {
        let wrapper = env::var_os("CODEX_CLI_PATH")
            .context("app-server proxy helper 缺少受控 Codex 包装器路径")?;
        let target = env::var_os("CODEY_CODEX_CLI_WRAPPER_TARGET")
            .context("app-server proxy helper 缺少包装器的真实 Codex 目标")?;
