...
impl WorkflowHost {
    ...
    pub fn apply_config(&self, config: WorkflowConfig) {
        self.set_global_mode(config.enabled && config.global_mode);
        if !config.enabled {
            ...
            match manager.recover().await {
                ...
                Err(error) => {
                    crate::error_log::record_failure(
                        ...
                        "recover_workflows_after_startup",
                        error.to_string(),
                        json!({ "recoverable": true }),
                    );
                    host.recovery_started.store(false, Ordering::Release);
